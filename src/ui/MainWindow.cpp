    ...
    , m_timer(new Timer(this))
{
    ...
    // 连接信号
    connect(m_timer, &Timer::timeChanged, this, &MainWindow::onTimeChanged);
    connect(m_timer, &Timer::stateChanged, this, &MainWindow::onStateChanged);
    connect(m_timer, &Timer::completed, this, &MainWindow::onTimerCompleted);
...
void MainWindow::onStateChanged(Timer::State state) {
    switch (state) {
    case Timer::Idle:
...
void MainWindow::onTimerCompleted(const QString& message) {
...
void MainWindow::onStartClicked() {
    if (m_timer->state() == Timer::Idle) {
        m_timer->start();
    } else if (m_timer->state() == Timer::Paused) {
        m_timer->resume();
    }
}
